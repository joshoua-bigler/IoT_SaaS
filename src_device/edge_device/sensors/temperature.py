...
class Temperature(DisturbanceSimulator):
  ...
  def _load_model(self, model: ARIMAResults = None):
    try:
      ...
      self.model = model if model else ARIMAResults.load(f'{model_path}/temperature_model_v2.pkl')
    except FileNotFoundError:
      raise FileNotFoundError(f'Temperature model file not found at {model_path}/temperature_model_v2.pkl')
